...
- `@Valid`로 요청 데이터 검증
- `ResponseEntity`로 HTTP 상태코드 명시적 제어 (등록 201, 조회/수정 200, 삭제 204)
...
- `WorkoutForm`/`WorkoutUpdateForm`에 러닝·복싱 전용 필드를 모두 두고, `@AssertTrue` 커스텀 검증으로 타입-필드 정합성 체크 (러닝인데 distanceKm 없으면 400, 복싱인데 rounds 없으면 400)
- `WorkoutResponse`/`WorkoutSummaryResponse`는 `instanceof` 분기로 타입별 필드를 노출
